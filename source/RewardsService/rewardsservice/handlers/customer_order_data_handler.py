    ...
    def post(self) -> str:
        ...
        class Customer:
            ...
            def __init__(self):
                ...
                self.reward_tier = str()
                self.reward_tier_name = str()
                self.next_reward_tier = str()
                self.next_reward_tier_name = str()
                self.next_reward_tier_progress = float()
        ...
        customer.reward_points = self.calculateRewardPoints(order_total, customer.reward_points)
        customer.reward_tier = self.calculateRewardTier(customer.reward_points)
        customer.reward_tier_name = self.generateRewardTierName(customer.reward_tier)
        customer.next_reward_tier = self.calculateNextTierAndNextTierName(customer.reward_tier)[0]
        customer.next_reward_tier_name = self.calculateNextTierAndNextTierName(customer.reward_tier)[1]
        customer.next_reward_tier_progress = self.calculateNextTierProgress(customer.reward_points)
        ...
        res = customer_order_data_collection.find({"Email Address":"test@test.com"})
