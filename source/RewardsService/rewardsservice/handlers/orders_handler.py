...
class OrdersHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        db = client["Rewards"]
        customer_record = OrderCalculator.get_email_record(
            self, self.get_body_argument("email address"), db)
        if customer_record is not None:
            OrdersHandler.add_order_to_existing_customer(
                self, db, customer_record)
        else:
            OrdersHandler.create_new_customer_record(self, db)
        customer = db.orders.find_one(
    ...
    def create_new_customer_record(self, db):
        reward_points = OrderCalculator.get_reward_points(
            self, self.get_body_argument("order total"))
        tier_record = OrderCalculator.get_tier_record(self, reward_points)
        next_tier_record = OrderCalculator.get_next_tier_record(
            self, tier_record["tier"])
        percentage_to_next_tier = OrderCalculator.get_percentage_to_next_tier(
            self, reward_points, next_tier_record["points"])
        db.orders.insert({
            "emailAddress": self.get_body_argument("email address"),
    ...
    def add_order_to_existing_customer(self, db, customer_record):
        reward_points = customer_record["rewardPoints"] + OrderCalculator.get_reward_points(
            self, self.get_body_argument("order total"))
        tier_record = OrderCalculator.get_tier_record(self, reward_points)
        next_tier_record = OrderCalculator.get_next_tier_record(
            self, tier_record["tier"])
